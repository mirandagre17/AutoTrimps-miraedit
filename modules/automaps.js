...
//            if (!game.portal.Overkill.locked && game.stats.cellsOverkilled.value)
    ...
    for (var map in game.global.mapsOwnedArray) {
        ...
        if (theMap.noRecycle && getPageSetting('RunUniqueMaps')) {
            ...
            //TODO Spire II+??
            if (getPageSetting('RunBionicBeforeSpire') && (game.global.world == 400) && theMap.name.includes('Bionic Wonderland')){
                ...
                if (bionicnumber == game.global.bionicOwned && bionicnumber < 19){
                    selectedMap = theMap.id;
                    break;
                }
                if (shouldDoSpireMaps && theMap.name == 'Bionic Wonderland XIX'){
                    selectedMap = theMap.id;
                    break;
                }
            }
